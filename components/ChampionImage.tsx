        ...
        label={
          <Box padding={1}>
            ...
            <Grid
              ...
              columnGap={4}
            >
              {getChampionStatsAsKeyValueArray(champion)
                .filter((stat) =>
                  stat[0].includes("level") || stat[0].includes("regen")
                    ? false
                    : true
                )
                .map((value, index, array) => (
                  <GridItem
                    colSpan={array.length - 1 == index ? 2 : 1}
                    key={value[0]}
                  >
                    <HStack
                      gap={0.5}
                      alignItems={"center"}
                      justifyContent={
                        array.length - 1 == index ? "center" : "start"
                      }
                    >
                      <Image
                        alt={value[0]}
                        src={`/stats/${value[0]}.webp`}
                        width={3.5}
                        height={3.5}
                      />{" "}
                      <Text>: {value[1]}</Text>
